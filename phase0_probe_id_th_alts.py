"""
Phase 0 — ID/TH alternative-source probe (Step 3a/3b).

Per the 2026-06-18 plan:
  ID — ShopeeFood as alternative to the JS-shell GoFood pages.
  TH — LineMan Wongnai per-restaurant pages (different route than
       the listing pages that bailed in the prior Track B probe).

Same yields-table format as coverage_report.md. Adds the Block column
inherited from coverage_report_track_b.md for fast captcha/tiny-page
detection.

Output: coverage_report_id_th_alts.{csv,md}
"""
import csv
import os
import random
...
PROBES = [
    # Indonesia ShopeeFood — Shopee's food-delivery vertical.
    ('Indonesia',  'formal', 'shopeefood SP',     'shopee.co.id/m/shopeefood-*',           'IDR'),
    ('Indonesia',  'formal', 'shopeefood',        'shopeefood.co.id/*',                    'IDR'),
    ('Indonesia',  'formal', 'shopeefood jkt',    'shopeefood.co.id/jakarta/*',            'IDR'),
    # Thailand LineMan Wongnai — per-restaurant pages (not listings).
    ('Thailand',   'formal', 'wongnai restaurants','wongnai.com/restaurants/*',            'THB'),
    ('Thailand',   'formal', 'wongnai bangkok',   'wongnai.com/bangkok/*',                 'THB'),
    ('Thailand',   'formal', 'lineman wongnai',   'lineman.line.me/r/wongnai/*',           'THB'),
    ('Thailand',   'formal', 'wongnai restaurant detail',
                              'wongnai.com/restaurants/*-*',                               'THB'),
]

CURRENCY_REGEXES = {
    'IDR': re.compile(r'Rp\.?\s?\d+(?:[.,]\d{3})*'),
    'THB': re.compile(r'(?:฿|THB|baht)\s?\d+(?:[.,]\d{2})?', re.I),
}
...
def sample_page(timestamp, orig_url, currency_re):
    ...
    try:
        ...
        if len(text) < 400:
            ...
            block = 'captcha'
        return {
            'kb': len(r.content) // 1024, 'price_hits': n_price,
            'json_ld': has_ld, 'next_data': has_nd, 'block': block,
        }, None
    except Exception as e:
...
def main():
    print(f"Phase 0 ID/TH alts probe — {len(PROBES)} pairs", flush=True)
    results = []
    rnd = random.Random(20260618)

    for i, (country, sector, platform, pat, cur) in enumerate(PROBES, 1):
        print(f"[{i:>2}/{len(PROBES)}] {country:<10} {platform:<28} {pat}",
              flush=True)
        ...
        cre   = CURRENCY_REGEXES.get(cur, re.compile(r'\d+'))
    ...
    csv_path = os.path.join(BASE, 'coverage_report_id_th_alts.csv')
    ...
    with open(csv_path,'w',newline='') as fh:
        w = csv.DictWriter(fh, fieldnames=fns); w.writeheader(); w.writerows(results)
    print(f"\nCSV → {csv_path}", flush=True)

    md_path = os.path.join(BASE, 'coverage_report_id_th_alts.md')
    with open(md_path,'w') as fh:
        fh.write(f"# Phase 0 — ID/TH alternative-source probe\n\n")
        fh.write(f"Generated: {datetime.now().isoformat()}\n  \n")
        fh.write(f"Window: {WINDOW_FROM} → {WINDOW_TO}\n  \n")
        fh.write("Probes alternative routes to escape the dead GoFood / "
                 "GrabFood paths identified in `coverage_report_track_b.md`.\n\n")
        fh.write("## Matrix\n\n")
        ...
        fh.write("\nIf both ID and TH bail again, the documented plan is "
                 "to ship them as floor-only countries — see "
                 "the user direction 2026-06-18.\n")
