...
Consumer price indices in developing economies rely on infrequent collection, and algorithmic alternatives like the MIT Billion Prices Project exclude services and the informal food economy (40–65 % of food expenditure in emerging markets). This paper's contribution is a **scalable, open-source method** for collecting restaurant and informal-vendor menu prices and constructing a UICPI (Unified Independent-Chain Price Index), with an eight-country panel as proof of concept.

The pipeline combines Phase 0 yield probes against Wayback CDX archives to triage sources before scraping, per-source extractors matched to each platform's HTML pattern, a live scraper, and a bail-decision audit trail. Index construction uses the matched-model restaurant-median method; Granger testing follows Cavallo and Rigobon (2016).

Across 41,263 price observations, the method yields one result of interest: in the United States (n = 31), UICPI Granger-leads headline CPI at an exact one-month calendar lag (F(1, 28) = 4.20, analytic p = 0.0499; circular-block permutation p = 0.069, naive shuffle p = 0.052), strengthening under a causal forward-fill robustness check (F(1, 35) = 9.05, p = 0.0048, n = 38). The US result is the most numerically interesting in the panel but should be read as suggestive rather than confirmatory, given the small sample, the multiple-testing problem in §8, and the block-permutation caveat. India (n = 47) and Malaysia (n = 30) return clean nulls (F = 0.521, p = 0.474; F = 0.111, p = 0.742); five countries remain below n = 24. The signal is one of *timing*, not magnitude.
...
Section 3 places this against prior alternative-price-index work. Section 4 details the method. Section 5 reports the per-country data. Section 6 presents the United States Granger result, the India and Malaysia null results, and the status of the remaining five countries. Section 7 discusses what the US finding implies and what the emerging-market nulls imply. Section 8 catalogues the honest limitations. Section 9 outlines the going-forward path.
...
The extractors are intentionally narrow because the archived HTML's structure varies sharply by platform vintage; five patterns recur, each matched to a distinct source layer — DOM markers (Menulog AU), Schema.org JSON-LD (MenuPages US), Next.js `__NEXT_DATA__` (GrabFood SG), embedded body-HTML JSON (Deliveroo UK), and a restaurant-level cost-for-two regex (Zomato NCR + Jakarta). Only the last of these yields restaurant-aggregate rather than item-level data, which is why the index treats IN and ID as coarser observations. Per-extractor markup contracts, fallback behaviour, and sample yields are given in **Appendix A**.
...
| **Permutation p (circular block, b=5) — primary** | **0.069** |
| Permutation p (naive full shuffle, 1000 draws) — secondary | 0.052 |
| **Lead time** | **1 month** |
...
**The Granger test is borderline, not a clean rejection of independence.** The analytic F-test places p just under 0.05 (p = 0.0499), while the permutation checks place it just outside. The two permutation designs are not equally informative, and the distinction matters for how the result should be read.

A *naive full shuffle* randomly reorders the UICPI series without restriction. Doing so destroys exactly the serial dependence structure that Granger causality is defined over: the null distribution it generates is one in which the predictor has no autocorrelation at all, not one in which it is merely uninformative about CPI. Because the observed series *is* serially dependent, the naive shuffle tests a null that differs from the null of interest, and it is therefore a weaker robustness check on its own. A *circular-block permutation* resamples contiguous blocks (here b = 5 months) and wraps around the series end, preserving local autocorrelation within each block while breaking the alignment between UICPI and CPI. This is the more methodologically appropriate resampling test for a lead–lag question of this kind, and **p = 0.069 (circular block, b = 5) is the primary permutation-based figure reported here**. The naive shuffle (p = 0.052) is retained as a secondary, weaker check; it happens to agree closely with both the block result and the analytic test, which is reassuring but should not be read as independent corroboration.

The result strengthens under a causal forward-fill robustness check that fills single-month menu gaps using only past information (F(1, 35) = 9.05, p = 0.0048, n = 38); a midpoint-interpolation variant agrees in direction but carries a look-ahead caveat and is not treated as robustness-confirming. Taken together, the US result (F = 4.20, p = 0.0499) is the most numerically interesting in the panel but should be read as suggestive rather than confirmatory, given the small sample (n = 31), the multiple-testing problem in §8, and the block-permutation caveat above. The finding is a *timing* signal: UICPI changes precede CPI changes by one month. No claim is made here about the magnitude of the eventual CPI change.
...
|---|---:|---|---|---|
| Australia | 23 | `[quarterly-interp]` | 1 month short — UICPI has 24 distinct months but the 2026-06 live snapshot is outside the AU CPI window (which ends 2026-04) | Late July 2026, when ABS publishes Q2 CPI via OECD SDMX |
...
The eight-country panel yields one borderline positive result (United States) and two informative nulls (India, Malaysia). Four implications are worth pulling apart.

### 7.1 Timing, not magnitude — a suggestive result

The US result (F = 4.20, p = 0.0499) is the most numerically interesting in the panel but should be read as suggestive rather than confirmatory, given the small sample (n = 31), the multiple-testing problem in §8, and the block-permutation caveat above (§6.1): the circular-block permutation, which preserves the serial dependence the test is defined over, returns p = 0.069. The Granger test is about predictive sequence: knowing past UICPI movements improves forecasts of current CPI changes. This paper does not make a companion magnitude claim in this draft — a pass-through regression on the calendar-true series has not been re-estimated, and the prior magnitude estimate (computed on the now-deprecated gap-mixed CPI construction) is not carried forward. UICPI is therefore presented strictly as a candidate *leading indicator* — a claim about the order of events — with no claim, in either direction, about the size of the eventual CPI movement. The substantive content is in the order of events, read at the strength the block permutation actually supports.
...
1. **Single, suggestive result.** One country, n = 31, analytic p = 0.0499, with the primary circular-block permutation at p = 0.069 and the secondary naive full shuffle at p = 0.052 (§6.1). The US result (F = 4.20, p = 0.0499) is the most numerically interesting in the panel but should be read as suggestive rather than confirmatory, given the small sample (n = 31), the multiple-testing problem in item 9 below, and the block-permutation caveat. Replication in AU and UK over the next 12 months will tell us whether the US case is generic or country-specific.
...
## Appendix A — Per-platform extractor specifications
...
## Appendix B — Extraction defects found and corrected

Referenced from §5.2. Two parser defects were identified and fixed before the analysis series was built. Both are recorded here with row counts and pre/post yields so the correction is auditable.

- **TripAdvisor tier-marker purge (2026-06-17)**. TripAdvisor `priceRange` markers `$` / `$$` / `$$$` / `$$$$` were initially stored as integer ordinals 1–4 in the `price` column, contaminating the price distribution with values that are not prices. On 2026-06-17, 1,648 such rows were deleted from `prices` (TH 279, UK 293, US 277, IN 237, AU 221, MY 188, ID 118, SG 35); the scraper was updated to skip the tier path; the index builder's tier-mask filter was removed as redundant. Summarised in §4.6.
